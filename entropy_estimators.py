...
def midd(x, y, base=2):
    """ Discrete mutual information estimator
        ...
        I (X; Y) = H(X) + H(Y) - H(X, Y)
        		 = H(X) - H(X|Y)
    """
    #assert len(x)==len(y), 'Distinct dimensions: %d <> %d' % (len(x), len(y))
    mi = entropyd(x, base) + entropyd(y, base) - entropyd(zip(x, y), base)
    #assert mi >= 0, 'Negative MI = %f, and len= %d, %d' % (mi, len(x), len(y))
    return mi
...
def cmidd(x, y, z):
    ...
    """
    #assert len(x)==len(y) and len(x)==len(z), 'Distinct dimensions: %d, %d, %d' % (len(x), len(y), len(z))
...
def conditional_entropy(x, y): # == ee.centropyd(S, E) # H(X, Y) - H(Y)
	# H(X|Y) = H(X) - I(X; Y) 
	#assert len(x)==len(y), 'Distinct dimensions: %d <> %d' % (len(x), len(y))
	return entropyd(x) -  midd(x, y)
